/**
 * Storybook: design-spec–generated Card from `lib/react/synapse/card`
 *
 * Anatomy:
 *   SynapseCard → Header (Title cluster + AdditionalFilter? + kebab?) → Body → Footer?
 *
 * Composition: lib `SynapseIcon` (kebab `overflow-menu-dots` in `var(--color-icon-brand-base)`;
 * key-value icons), `SynapseButton` tertiary for footer actions.
 * Theme: components/synapse-theme.css
 * Spec: components/synapse/card/design-spec.md
 */
import React from "react";
import type { Meta, StoryObj } from "@storybook/react";
import "../../../../components/synapse-theme.css";
import {
  SYNAPSE_CARD_DOCS_DESCRIPTION,
  SYNAPSE_CARD_SOURCE_CODE,
} from "./synapse-card.developer-usage";
import {
  SynapseCard,
  SynapseCardKeyValueContent,
  SynapseCardTextContent,
  type SynapseCardAction,
  type SynapseCardMenuOption,
  type SynapseCardProps,
} from "@synapse/react/card";

const DESIGN_SPEC_PATH = "components/synapse/card/design-spec.md";

/** Spec Accurate Design story defaults — design-spec Composition & API. */
const SPEC_MENU_OPTIONS: SynapseCardMenuOption[] = [
  { value: "edit", label: "Edit" },
  { value: "duplicate", label: "Duplicate" },
  { value: "delete", label: "Delete" },
];

const SPEC_ACTIONS: SynapseCardAction[] = [
  { id: "a1", label: "Action" },
  { id: "a2", label: "Action" },
];
...
const FIGMA_TEXT_BODY = (
  <SynapseCardTextContent sectionTitle="Section Title">
...
const meta: Meta<SynapseCardProps> = {
  ...
  parameters: {
    ...
    docs: {
      ...
      description: {
        component: SYNAPSE_CARD_DOCS_DESCRIPTION,
      },
      source: {
        type: "code",
        language: "tsx",
        code: SYNAPSE_CARD_SOURCE_CODE,
      },
    },
  },
  argTypes: {
...
export default meta;
type Story = StoryObj<SynapseCardProps>;
...
export const FigmaCardMainMatrix: Story = {
  ...
  render: () => (
    ...
    >
      ...
      <SynapseCard title="Card Title" showButtons={false} showOverflowMenu={false}>
        {FIGMA_TEXT_BODY}
      </SynapseCard>
      <SynapseCard
        title="Card Title"
