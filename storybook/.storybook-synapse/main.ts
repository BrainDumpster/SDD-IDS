/**
 * design-sync Synapse-only Storybook config.
 *
 ...
 * index to the canonical Synapse stories in `src/components/lib-generated/Synapse*.stories.tsx`
 * — the ones that import from `@synapse/react/*` (i.e. `lib/react/synapse/`), which is what
 * the synced Synapse bundle ships.
 *
 * Everything else (vite aliases, decorators, staticDirs) is inherited from `../.storybook/main.ts`.
 * Sibling config: `../.storybook-ids/` does the same for IDS.
 */
...
const storyFiles = fs
  ...
  .filter((f) => /^Synapse.*\.stories\.tsx?$/.test(f))
