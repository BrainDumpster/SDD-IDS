...
const MIME = { '.html': 'text/html', '.js': 'text/javascript', '.mjs': 'text/javascript', '.css': 'text/css', '.json': 'application/json', '.png': 'image/png',
  // Without these, assets are served as application/octet-stream. Chromium then
  // refuses to paint them as CSS images, so `mask-image: url(<svg>)` icon systems
  // photograph blank on the storybook panel while a data-URI preview looks correct.
  '.svg': 'image/svg+xml', '.jpg': 'image/jpeg', '.jpeg': 'image/jpeg', '.gif': 'image/gif', '.webp': 'image/webp', '.avif': 'image/avif',
