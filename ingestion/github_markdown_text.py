...
def extract_github_markdown_text_by_section(
    html: str,
    source_name: str,
    *,
    max_chars_per_line: int = 1200,
...
def extract_prose_from_markdown_file(
    mdx_text: str,
    source_label: str,
) -> dict[str, list[str]]:
    ...
    if _looks_like_html_fragment(cleaned):
        for k, v in extract_github_markdown_text_by_section(cleaned, source_label).items():
            merged[k].extend(v)
    else:
        html = mdx_markdown_to_html(cleaned)
        for k, v in extract_github_markdown_text_by_section(html, source_label).items():
            merged[k].extend(v)
