...
- `footer buttons`: must be rendered from the IDS Button contract defined in `components/ids/button/design-spec.mdx` (variant, size, disabled, focus-visible, and event behavior must inherit IDS Button rules).
