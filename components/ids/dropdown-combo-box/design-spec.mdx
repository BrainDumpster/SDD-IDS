...
| Option row | hover | `var(--color-background-brand-lighter)` | none | neutral text |
...
| Option rows | default/hover/selected | semantic token resolved | semantic token resolved | semantic token resolved |
...
- Menu rows use tokenized row padding and brand-lighter selected/hover backgrounds.
