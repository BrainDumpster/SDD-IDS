...
- **Background mode fix (2026-05-24)**: Fixed `background="none"` to use `transparent` background instead of `var(--color-background-component)`. This ensures the pagination root has no background color when the "none" mode is selected, allowing it to blend with the parent container's background.
