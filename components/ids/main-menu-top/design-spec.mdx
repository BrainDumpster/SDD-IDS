...
- **Design system:** IDS
- **Category:** Navigation
- **Spec path:** `components/ids/main-menu-top/design-spec.mdx`
...
3. **`MainMenuTopDropdown`** (per item with `menuOptions`) — anchored below trigger; reuses IDS **Dropdown: Single-select** menu contract (`components/ids/dropdown-single-select/design-spec.mdx`, Figma `12579:19717` menu + `12380:16525` options)
...
- **Dropdown contract:** option list styling, hover/selected/disabled behavior, and 44px min hit target follow **`components/ids/dropdown-single-select/design-spec.mdx`** (menu elements `12579:19717`, options `12380:16525`). Radio rows in Figma samples are hidden in masthead menus — omit unless product enables `showRadio`.
...
- **Dropdown dependency:** `components/ids/dropdown-single-select/design-spec.mdx`
