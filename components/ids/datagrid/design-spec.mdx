...
- Last verified: **2026-05-25** (combobox-filter multiselect `44360:147581`, combobox-filter single-select `44360:179074`, dropdown-singleselect `44360:179201`, dropdown-multiselect `44360:179348`, column filter types `37822:91069`, numeric filter `44360:182265`, default text filter `37822:91073` via Figma MCP; prior grid chrome **2026-05-14**)
- Nested specs (codegen must delegate, not reimplement): `components/ids/checkbox/design-spec.mdx`, `components/ids/pagination/design-spec.mdx` (footer), detail panel datagrid attach mode (see `IdsDetailPanel` / product `def-new-dg-detail`)
- Reference implementation (Storybook / React parity target): `storybook/src/components/IdsDataGrid.tsx`, `IdsDataGrid.module.css`, `IdsDataGridDefaultStoryHost.tsx`, `IdsDataGridSelectionCheckbox.tsx`
...
| **Selection** | IDS Checkbox spec | All checkboxes (header + rows) via `components/ids/checkbox/design-spec.mdx` |
...
1. `SelectAllColumn?` (**`48px`**, when `multiselect: on`)
...
| `SelectAllColumn` / `RowSelectionCell` | checkbox column templates | **IDS Checkbox** — indeterminate select-all |
...
| Numeric filter host | `IdsDataGridNumericFilterStoryHost` | Wires `filterActive` + `IdsDataGridTypeNumericFilterPanel` |
| Numeric filter panel | `IdsDataGridTypeNumericFilterPanel` | Figma `44360:182265`; model in `IdsDataGridNumericFilter.ts` |
| Selection | `IdsDataGridSelectionCheckbox` | `Checkbox` `density="datagrid"` |
...
- **Panel borders:** **left + bottom + right** **`1px`** **`var(--color-border-accessible)`**; elevation via stacked **`shadow-drop-shadow-*`** semantic tokens.
...
- **`FilterPanelBody` slot:** **`column.filterPanel`**. Search is **optional**. Checkbox lists must follow **`components/ids/checkbox/design-spec.mdx`**.
...
- **Filter types delegate to IDS component specs** for their inner controls (checkboxes → `components/ids/checkbox/design-spec.mdx`, dropdowns → `components/ids/dropdown-single-select/design-spec.mdx`, radio buttons → `components/ids/radio-button/design-spec.mdx`, text fields → `components/ids/text-box/design-spec.mdx`). Codegen must use the IDS component for each control, not custom markup.
...
| `Date` | Date picker filter | Planned |
| `Date and Time` | Date-time filter | Planned |
| `Numeric` | Operator radio list + value field(s) | Spec'd below |

Reference nodes: **`37822:91069`** (matrix), **`44386:290322`** (`Type=Numeric` instance).
...
Multiselect combobox filter with search, Select All / Clear All, and scrollable checkbox option list. Tokens and interaction states follow **`components/ids/dropdown-combo-box/design-spec.mdx`** (`mode: combobox-multi`).
...
- **Select All / Clear All row:** `var(--padding-padding-8)` vertical / `var(--padding-padding-16)` left padding; bottom border `var(--border-width-border-default)` solid `var(--color-border-accessible)`.
  - Checkbox: `16×16`, `var(--corner-radius-radius-2)` corners, border `var(--color-border-accessible)`. Delegate to `components/ids/checkbox/design-spec.mdx`.
...
- **Option row:** `var(--padding-padding-10)` vertical / `var(--padding-padding-16)` horizontal, `var(--spacing-space-8)` gap between checkbox and label, min-height `40px`. Checkbox `16×16` per `components/ids/checkbox/design-spec.mdx`. Label: Body 2 Medium, `var(--color-text-neutral)`, overflow ellipsis.
...
Single-select combobox filter with search and scrollable option list (text-only rows, no radio or checkbox controls). Tokens and interaction states follow **`components/ids/dropdown-combo-box/design-spec.mdx`** (`mode: combobox-single`).
...
Simple single-select filter with a scrollable plain-text option list — no search row. Structurally identical to Combobox-SingleSelect minus the `SearchRow`. Codegen must delegate option row tokens and interaction states to **`components/ids/dropdown-single-select/design-spec.mdx`**.
...
Delegate to **`components/ids/dropdown-single-select/design-spec.mdx`** States tables:
...
Implementations must use the IDS Dropdown Single-Select component (`components/ids/dropdown-single-select/design-spec.mdx`) for option row rendering, tokens, and interaction states — not custom markup.

---

### Filter type: Dropdown-MultiSelect (`Multi-select`, `44360:179348`)

Multiselect filter with Select All / Clear All row and scrollable checkbox option list — no search row. Structurally identical to Combobox-Multiselect minus the `SearchRow`. Codegen must delegate checkbox rendering to **`components/ids/checkbox/design-spec.mdx`** and option row patterns to IDS dropdown multi-select conventions.
...
- **Select All / Clear All row:** `var(--padding-padding-8)` vertical / `var(--padding-padding-16)` left padding; bottom border `var(--border-width-border-default)` solid `var(--color-border-accessible)`.
  ...
  - "Select All" label: Body 2 Medium, `var(--color-text-neutral)`.
  - "Clear All" action: Body 2 Medium, `var(--color-text-brand-strong)` (enabled) / `var(--color-text-disabled)` (disabled when nothing selected). Padding `var(--padding-padding-2)` vertical / `var(--padding-padding-16)` horizontal.
- **Option row** (`.Dropdown-Elements-MultiSelect-Options`): `var(--padding-padding-10)` vertical / `var(--padding-padding-16)` horizontal, `var(--spacing-space-8)` gap between checkbox and label, min-height `40px`. Checkbox `16×16` per `components/ids/checkbox/design-spec.mdx`. Label: Body 2 Medium, `var(--color-text-neutral)`, overflow ellipsis, `white-space: nowrap`.
...
Implementations must use the IDS Checkbox component (`components/ids/checkbox/design-spec.mdx`) for option row checkboxes. Option row layout and tokens follow IDS dropdown multi-select conventions.
...
- **Operators** (single-select radio group, labels from Figma):
  ...
  2. `Equals` / `Does not equal` / `Greater than` / `Greater than equal to` / `Less than` / `Less than equal to` — show one value block: `padding-left: 40px`, `padding-right: var(--padding-padding-16)`, `padding-bottom: var(--padding-padding-8)`, gap `16px` between rows.
     - **Text field:** height `40px`, border `var(--border-width-border-default)` solid `var(--color-border-accessible)`, horizontal padding `var(--padding-padding-16)`, vertical padding `var(--padding-padding-8)`. Width: `242px` sample, flex `1` when unit dropdown present. Delegate states to `components/ids/text-box/design-spec.mdx`.
...
- **Unit dropdown (optional):** delegate to `components/ids/dropdown-single-select/design-spec.mdx` — field height `40px` (Large), padding `var(--padding-padding-10)` vertical / `var(--padding-padding-16)` horizontal, border/background/text tokens per that spec. Caret icon `arrow-drop-tri-caret`. Placeholder `"-Select-"`.

#### Delegated specs

- **Radio control:** `components/ids/radio-button/design-spec.mdx` — `16×16` control, dot `var(--color-icon-brand-base)`, selected border `var(--color-border-brand-base)`, hover border `var(--color-border-strong)`, focus-visible ring `1px var(--color-border-brand-base)` offset `2px`.
- **Text field:** `components/ids/text-box/design-spec.mdx` — hover `var(--color-border-strong)`, focus `var(--color-border-brand-base)`, focus-visible outer ring `var(--color-border-brand-base)` offset `4px` with `border-radius: 4px`.
- **Unit dropdown:** `components/ids/dropdown-single-select/design-spec.mdx` — all trigger and popup token contracts per that spec.
...
- `filterActive` when `operator !== 'all'` and required value(s) are non-empty
...
- **Row / cell body chrome (Figma `.Row/Cell: States and styling`, `37721:114580`):**
  ...
  - **Select-all column (`37721:114682` header variant, `37721:113988` grid column):**
    - **Column width:** **`48px`** (`<colgroup>` only; Figma `37721:113988` width **48**).
    - **Header (`37721:114682`, `colorAndBorder=true`, `type=Selection`):** **`48px`** total column width; **`padding: 16px`** horizontal + **`padding-block: var(--selection-header, 16px)`**; centered **16×16** checkbox (`8505:14304`); **`.Header: Styling`**: **`var(--color-background-gray-neutral-lighter)`**, **1px** top + bottom **`var(--color-border-light)`** — **no** extra leading vertical rule on this column (first data column owns the **1×24px** rail).
    - **Body cell (`37721:113988` `.Cell Item`):** **`48px`** wide, **`40px`** row height, **`padding: 12px 16px`**, **16×16** checkbox — **no** leading full-height edge on the selection column in Storybook parity.
...
- **`colgroup` / table width vs. side “gaps” (implementation note):**
  ...
  - **Full-width vs. horizontal scroll contract (Figma `37721:113987` *Rows and Columns*):**
    ...
    - Set table **`min-width`** to the sum of **pinned + data column widths** (selection **`48px`** when `multiselect`, each data column at least **`90px`** or its declared/default width, settings **`40px`**).
...
8. **Chrome cells (`SelectAll`, `SettingsColumn`):** `<colgroup>` + matching **`th`/`td`** **`width` / `min-width` / `max-width`** (**`48px`** / **`40px`** only — same constants). Inner hosts use fixed column width + **`overflow: hidden`**. **No** leading **`::before`** divider on selection/settings chrome (divider before **Name** comes from the first **data** header rail only). Datagrid checkbox control **16×16** (`field[data-density="datagrid"]`).
...
|---|---|---|---|
| Select-all | **`37721:113988`** | **`37721:114682`** | **48px** |
...
**Select-all (`37721:114682` / `37721:113988`):**
...
| Checkbox | **16×16** — IDS Checkbox `density="datagrid"` (`field[data-density="datagrid"] .root { width: 16px; height: 16px }`) |
| Body row height | **40px** |
| Body padding | **`12px 16px`** (`var(--selection-cell, 12px)` vertical) |
| Leading chrome divider | **None** on selection column — divider before **Name** is the **first data** header leading **1×24px** rail only |
...
```
[SelectAllColumn? 48px] → [DataColumn × N fixed Npx] → [GrowColumn auto] → [SettingsColumn 40px]
```
...
// colgroup:
// multiselect ? <col width="48px" /> : null
...
- Filter:
  ...
  - When a filter uses **checkboxes** (e.g. multiselect value list), controls **must** follow **`components/ids/checkbox/design-spec.mdx`** (anatomy, tokens, states, focus-visible, indeterminate → checked rule).
- Selection:
  ...
  - **All** selection checkboxes (header select-all + per-row) **must** render through the IDS **Checkbox** component contract (`components/ids/checkbox/design-spec.mdx`): **16×16** control box, semantic tokens, focus-visible ring, **indeterminate** on select-all when partially selected; **no** native `<input type="checkbox">` in production implementations.
  - checkbox hit target may use compact density in **48px** selection column (control remains **16×16**).
- Detail panel (when `withDetailPanel` / `detailsPanel: attached`):
  ...
  - checkbox / sort / filter / resize controls use **`stopPropagation`** so they do not toggle the panel.
...
interface DatagridProps {
  ...
  multiselect?: boolean;                    // default false (Storybook spec story: true)
...
| `onSelectionChange` | `(selectedRowKeys: string[])` | Checkbox / select-all |
...
- `multiselect: true`, `withDetailPanel: true`, `headerColorAndBorder: true`, `rowVerticalIndicator: true`, `columnResizeEnabled: true`, `readOnly: false`, `pageSize: 6`, `viewMode: "table"`
...
**`SelectAllColumn` / selection `<th>` / `<td>`** (chrome — keep `display: table-cell`):

```
col [width 48px]
th|td.selectionColumn [width/min/max 48px, sticky left, padding 0, overflow hidden]
  div.selectionHeaderContent [width 100%, h 48, padding 16px block + 16px inline, flex center]
    IdsDataGridSelectionCheckbox → Checkbox density=datagrid 16x16
td.rowSelectionCell
  div.selectionRowContent [width 100%, h 40, padding 12px 16px, flex center]
    Checkbox 16x16
```
...
| `SelectAllColumn` | **48px** fixed | header **16+16+16** padding+checkbox; body **12×16** padding |
...
Variant matrix:
  ...
  - **Chrome + slack (codegen):** implement **Chrome columns & table slack (codegen-critical)** — grow **`auto`**, chrome **48**/**40** on `<colgroup>` + `th`/`td`, inner hosts **`width:100%`**, **16×16** checkbox, no **`width:0`**, no grow width in `<colgroup>` until user resize completes.
...
- Validation checklist:
  ...
  - [ ] **Select-all** **`th`** = **48px** in DevTools; inner host **`width:100%`** (no gap before Name divider); **16×16** datagrid checkbox; body **`12px 16px`**.
  - [ ] **Settings** **`th`** = **40px**; **16×16** gear centered; **no** leading chrome `::before` divider; body **`padding-block: 12px`**.
  - [ ] select-all and row checkboxes use IDS Checkbox (`components/ids/checkbox/design-spec.mdx`) — not native HTML checkboxes; select-all supports **indeterminate**.
  ...
  - [ ] **Column filter (checkbox lists):** options use IDS Checkbox spec (`components/ids/checkbox/design-spec.mdx`) without drift.
...
- IDS authoritative nodes used:
  ...
  - Select-all header: **`37721:114682`**; selection grid column: **`37721:113988`** (**48px**)
...
1. **Read** `components/ids/datagrid/design-spec.mdx` as sole source of truth; emit `spec_hash` in `storybook-generated/ids/src/spec-contracts/datagrid.spec-layer-hash.json`.
