...
- Nested specs (codegen must delegate, not reimplement): `components/ids/checkbox/design-spec.mdx`, `components/ids/date-picker/design-spec.mdx`, `components/ids/time-picker/design-spec.mdx`, `components/ids/radio-button/design-spec.mdx`, `components/ids/pagination/design-spec.mdx` (footer), detail panel datagrid attach mode (see `IdsDetailPanel` / product `def-new-dg-detail`)
...
| **Filter value lists** | IDS Checkbox spec | Multiselect filter checkboxes via `components/ids/checkbox/design-spec.mdx` (unchanged) |
...
- **Radio control:** `components/ids/radio-button/design-spec.mdx` — same as numeric/date-time filters; keyboard-only focus ring.
- **Date field:** `components/ids/date-picker/design-spec.mdx` — `40px` field height, `calendar-simple-16`, format hint `MM-DD-YYYY`.
...
- **Radio control:** `components/ids/radio-button/design-spec.mdx` — same contracts as numeric filter (`16×16`, brand dot). **Focus ring:** keyboard (`:focus-visible`) only; pointer selection must not show focus outline.
- **Date field:** `components/ids/date-picker/design-spec.mdx` — `40px` field height, `calendar-simple-16`, format hint `MM-DD-YYYY`.
- **Time field:** `components/ids/time-picker/design-spec.mdx` — `40px` field height, `time-clock-16`, format hint `HH:MM AM/PM`, 12h clock in filter context.
...
- **Row / cell body chrome (Figma `.Row/Cell: States and styling`, `37721:114580`):**
  ...
  - **Selection column (`37721:114682` header variant, `37721:113988` grid column):**
    ...
    - **Header (`37721:114682`, `colorAndBorder=true`, `type=Selection`):** **`48px`** total column width; **`padding: 16px`** horizontal + **`padding-block: var(--selection-header, 16px)`**; centered **16×16** control host:
      - **`selectionMode: multiple`:** **select-all** IDS Checkbox (`8505:14304` in Figma); **`aria-label`** e.g. “Select all rows on this page”; **checked** when every row on the **current page** is selected; **indeterminate** when some but not all page rows are selected (per `components/ids/checkbox/design-spec.mdx` indeterminate → checked rule).
...
| Body control (`single`) | **16×16** IDS Radio Button per row (`components/ids/radio-button/design-spec.mdx`) |
| Body control (`multiple`) | **16×16** IDS Checkbox per row (`components/ids/checkbox/design-spec.mdx`) |
...
- Row selection:
  ...
  - radios follow **`components/ids/radio-button/design-spec.mdx`**; all checkboxes (header + rows) follow **`components/ids/checkbox/design-spec.mdx`** (**16×16**, indeterminate on select-all when applicable).
...
- Accessibility contract:
  ...
  - **single:** row radios expose correct selected state within `RadioGroup` (`aria-checked` on each radio); radio semantics delegated to `components/ids/radio-button/design-spec.mdx`.
...
- Validation checklist:
  ...
  - [ ] **`selectionMode: single` + `showSingleSelectionRadio: on`:** empty header; row radios in `RadioGroup` per `components/ids/radio-button/design-spec.mdx`.
  ...
  - [ ] **`selectionMode: multiple`:** header **select-all** checkbox (**16×16**, indeterminate when partial current-page selection); row checkboxes per `components/ids/checkbox/design-spec.mdx`; select-all toggles **current page** rows only.
