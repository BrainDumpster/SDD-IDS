...
| Property | Value |
|---|---|
| Tile size | `148×125` (min-height; grows for a wrapped label) |
| Icon slot | **`32×32`** |
...
| Label max width | **`111px`** (2-column tile); Body 2. **Icon tile:** single line. **No-icon (text-only) tile:** wraps up to **3 lines** (no ellipsis). A label that still doesn't fit promotes the tile to full-width (see **Long-label reflow**). |
| Label padding | **`28px 0`** (icon and no-icon variants; `justify-content: center` keeps the label centred). Horizontal padding is `0` (Figma `padding-none`) |
| Tile fill (hover/press) | **Full `148×125` footprint**; divider siblings unchanged |

### Long-label reflow

Product names are never truncated in a tile. Each name is measured at the normal `111px` tile width to decide its layout:

- **Icon tile:** the name is a single line. If it does not fit on one line, the tile is promoted to **full-width** (its own row, `~296px`) so the name can wrap.
- **No-icon (text-only) tile:** the name wraps up to **3 lines**. If it needs more than 3 lines, the tile is promoted to **full-width**, where the name wraps across the wider tile (`max-width: 258px`).

A full-width tile occupies a whole product row; a normal tile packs two-per-row. The panel shows at most **`APP_LAUNCHER_MAX_PRODUCT_ROWS = 2`** product rows (**4 tiles**, per `42266:95081`); products pushed past that cap overflow into the **options list** (React implementation — see **Options region**). The measurement runs off-screen before the panel opens, so the packed layout is stable when it appears.

*(Angular parity note: the Angular composition has no options region, so it applies the 3-line wrap + full-width promotion but does not cap/overflow into options.)*
...
| Truncated-label tooltip | switchable via `useIdsOptionTooltip` (default `false`): `false` reveals the full text with the **native browser tooltip** (`title`); `true` uses the branded **`IdsTooltip`** (anchored to the label). Applies to any truncated option row, including products that overflowed the tile cap. |
