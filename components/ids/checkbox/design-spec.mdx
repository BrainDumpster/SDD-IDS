...
| Indeterminate | Focus-visible | `var(--color-background-component)` | `var(--color-border-brand-base)` + 2px focus ring offset | dash in `var(--color-icon-brand-base)` | `var(--color-text-neutral)` |
| Any | Disabled | `var(--color-background-gray-light)` for unselected/indeterminate; `var(--color-background-gray-base)` for checked | `var(--color-border-disabled)` | selected check uses `var(--color-icon-inverse)`; indeterminate dash uses `var(--color-icon-disabled)`; unchecked none | `var(--color-text-disabled)` |
## States (Dark Theme)
