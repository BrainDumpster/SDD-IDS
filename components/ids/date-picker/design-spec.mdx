...
- Calendar is a standalone bordered box — full border on all 4 sides (including top), separated from the input by `var(--spacing-space-4)` (4px) gap
...
| Selected with dropdown | `var(--color-background-component)` | `var(--color-border-brand-base)` | `var(--color-text-neutral-strong)` | `var(--color-icon-brand-base)` | Calendar popup open; input keeps its own blue border, popup is a separate panel with full `var(--color-border-accessible)` border and `4px` gap below input; format hint remains below the field |
...
| `FieldContainer.open` | border-color: `var(--color-border-brand-base)`, border-bottom: transparent, border-bottom-left-radius: 0, border-bottom-right-radius: 0 |
| `CalendarPopup` | bg: `var(--color-background-surface-2)`, border: `var(--color-border-brand-base)` 1px (no top border), shadow: Shadow 4 stack, padding: `var(--padding-padding-16)`, right-aligned, no top border-radius, margin-top: 0; format hint rendered as first child inside popup when open |
