...
- **Component map (canonical):** `data/ids-ai-component-figma-map.json` → entry `slug` **`segmented-button`** (`fileKey` **`rr8F0NGA6RNY9Z0cNeh3jh`**).
...
- **Main component set:** `8218:13149` — **`SegmentedButton-Main`** — **24** published symbols: `(Type=Text × # Options ∈ {2,3,4,5} × Option 2 State) ∪ (Type=Icon × # Options ∈ {2,3} × Option 2 State)` with **four** states each (**Inactive**, **Hover**, **Press**, **Focus**) on **Option 2** only while **Option 1** stays selected (verified via Figma MCP `get_metadata` on `8218:13149`, 2026-05-10).
...
- **Dark scenario content (instances):** `42113:67351` (same as **scenarios** node in `ids-ai-component-figma-map.json`)
...
- **Figma references used for validation:**
  ...
  - Icon list / tints: `https://www.figma.com/design/rr8F0NGA6RNY9Z0cNeh3jh/IDS-AI-Design-Library--Test-?node-id=10148-29563&m=dev`
- **Variant axes (from Figma component set — `get_design_context` on `8218:13149`):**
  - **Type:** `Text` | `Icon`
  - **# Options:** `"2"` | `"3"` | `"4"` | `"5"` when `Type=Text`; `"2"` | `"3"` when `Type=Icon`
  - **Option 2 State:** `Inactive` | `Hover` | `Press` | `Focus` — only the **second** segment’s interaction is varied; **Option 1** remains the selected segment in the matrix.
- **Usage (from Figma description):** Segmented control / view switcher; mutually exclusive selection.
- **Last live verification (2026-05-10):** Figma MCP `get_design_context` + `get_metadata` on `8218:13149`; `get_variable_defs` on `8218:13149` — confirms root `padding-2`, `spacing/space-2` gap, `radius-2`, `border-default` + `Color/Border/Accessible`, text cells `padding-8` / `padding-4`, selected segment `Color/Background/Controls/Brand-Base` + `Color/Border/Brand-Base`, icon cells `size-32`, glyph lane `16×14`, `Body 2`, hover/press surfaces `brand-lighter` / `brand-light`.
...
- **Text segment (standard density)**
  - Row frame height: **`32px`** per published **Text** symbols on `8218:13149` (e.g. `8218:13150`, `9015:22161`).
  - Cell padding: vertical `var(--padding-padding-4)`, horizontal `var(--padding-padding-8)` (Figma export: `padding-4` / `padding-8`).
  ...
  - **Sample-only frame widths** (Figma matrix instances — runtime width is container-driven): Text `# Options=2` → `260px`, `3` → `370px`, `4` → `500px`, `5` → `600px` (from `get_design_context` export on `8218:13149`).
- **Icon segment**
  - **Segment surface:** **`32px` × `32px`** square (`var(--sizing-size-32)`), `rounded-[var(--corner-radius-radius-2)]` (Figma: `10148:29576` / composed e.g. `8218:13156`).
  - **Published symbol bounds (sample):** **`70×36`** for `Type=Icon, # Options=2`, **`104×36`** for `# Options=3` — height **`36px`** includes root **`2px`** top/bottom padding plus **`32px`** cell row (`get_metadata` on `8218:13149`).
  - **Glyph frame inside the cell:** **`16px` × `14px`** artwork bounds (`Sizing/size-16` × `14` in variable defs); center within the `32px` square.
...
## Source Mapping
- **Component map:** `data/ids-ai-component-figma-map.json` → **`segmented-button`** (`nodeId` **`8218-13149`**, `scenariosNodeId` **`42113-67351`**, text/icon/icon-list element URLs as in that file).
- **Documentation board:** `42113:67341`
- **Main component set:** `8218:13149` (`SegmentedButton-Main`) — **24** variant symbols (`get_metadata` child `symbol` list, 2026-05-10).
- **Light scenario content:** `42113:67348` · **Dark scenario content:** `42113:67351`
- **Element frames:** `9015:20992`, `10148:29576`, `10148:29563`
- **Live verification (Figma MCP, 2026-05-10):**
  - `get_metadata` + `get_design_context` on `8218:13149` — variant axes, layout tokens, sample widths, icon `70×36` / `104×36` symbol frames.
  - `get_variable_defs` on `8218:13149` — semantic color/spacing/radius/typography bindings listed in **Tokens** / state tables.
