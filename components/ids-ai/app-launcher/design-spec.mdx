...
- **Figma (scenarios / usage):** https://www.figma.com/design/rr8F0NGA6RNY9Z0cNeh3jh/IDS-AI-Design-Library--Test-?node-id=42266-95085&m=dev
- **Figma (component / layout matrix):** https://www.figma.com/design/rr8F0NGA6RNY9Z0cNeh3jh/IDS-AI-Design-Library--Test-?node-id=13231-123761&m=dev
- **Figma (elements & states):** https://www.figma.com/design/rr8F0NGA6RNY9Z0cNeh3jh/IDS-AI-Design-Library--Test-?node-id=13231-109521&m=dev
- **Figma file key:** `rr8F0NGA6RNY9Z0cNeh3jh`
- **Validated nodes:** `42266:95085` (scenarios), `13231:123761` (component), `13231:109521` (elements / states)
- **Theme:** `components/ids-ai-theme.css` (canonical names per `components/ids-ai/root-spec.mdx`)
- **Last live verification:** 2026-05-09 — Figma MCP `get_metadata` on `42266:95085`, `13231:123761`, `13231:109521`; Figma MCP `get_variable_defs` on default tile `13231:109520` (bindings include `Color/Background/Surface 2`, `Color/Border/Accessible`, `Border Width/border-default`, `Color/Text/Neutral-Strong`, `Color/Icon/Neutral-Strong`, `Body 2`).

## Layout & Measurements
- Launcher surface outer size (from `AppLauncher-Main` component-set variants, node `13231:123761`):
  - **1 product:** `150px` × `127px` (compact; matches scenario instance `42266:95088`).
  - **2 products:** `298px` × `127px`.
  - **3 or 4 products:** `298px` × `254px`.
  - **8 products:** `298px` × `416px`.
- Default multi-product menu body width: `298px` (scenarios `42266:95085` use this for 2+ product frames).
...
- Surface stroke/shadow:
  - Border width: `var(--border-width-border-default)` with `var(--color-border-accessible)`.
...
|---|---|---|---|---|
| Launcher surface | default | `var(--color-background-surface-2)` | `var(--border-width-border-default) solid var(--color-border-accessible)` | n/a |
...
## States (Dark Theme)
Dark theme follows the same slot/state matrix as Light Theme, resolved exclusively via semantic tokens in `components/ids-ai-theme.css`. No hardcoded colors are permitted in implementation.
...
| `columns` | No | Product columns; samples use `2`. |
...
### Supported matrix
- `productCount`: `0 | 1 | 2 | 3 | 4 | 8` (surface dimensions follow the variant table in Layout & Measurements; `1` uses the `150×127` compact surface)
...
- `OptionRow`: mirrors IDS dropdown option contract for default/hover/press token behavior.
- `AppLauncherSurface`: `var(--border-width-border-default)` accessible border + Shadow 1 token stack.
...
### Icon primitive (codegen — logical API)
Framework-agnostic; concrete component name and import come from **codegen / design-system config**, not this spec.

| Slot | `shapeName` | `color` (logical) | `variant` (logical) | Notes |
|---|---|---|---|---|
| Launcher trigger | `grid-square-9-16` | `var(--color-text-white)` if `triggerVariant === masthead`, else `var(--color-text-neutral-strong)` | `inline` when implementation supports curated inline SVG for this slug (real `<svg>` in DOM); else `mask`. | Explicit token per trigger context. |
| Default product tile | `shield-encrypt-alt` | *(omit)* | `inline` when supported for this slug; else `mask`. | Tile host sets `color` so default vs press/selected icon tokens follow the state table via inheritance. |
| Custom product row | `iconSlug` from data | Omit unless spec pins icon color independent of tile state | Usually `mask` unless slug is on implementation allowlist | Fallback slug: `shield-encrypt-alt`. |
...
- [ ] Usage-level masthead anchor/open behavior aligns with scenario node `42266:95085`.
- [ ] Product and option selection callbacks emit deterministic payloads.
- [ ] Light/Dark styles remain semantic-token driven (`ids-ai-theme.css`).

## Source Mapping
| Source | Location |
|---|---|
| Codegen icon primitive mapping (SDD-IDS tooling) | `config/design_systems/ids-ai.yaml` → `codegen` (optional; not Figma-sourced) |
| IDS-AI root spec | `components/ids-ai/root-spec.mdx` |
| IDS baseline reference (anatomy parity) | `components/ids/app-launcher/design-spec.mdx` |
| Component map | `data/component-figma-map.json` → **App Launcher (IDS-AI)** |
| Scenarios / usage | https://www.figma.com/design/rr8F0NGA6RNY9Z0cNeh3jh/IDS-AI-Design-Library--Test-?node-id=42266-95085&m=dev (`42266:95085`) |
| Component details | https://www.figma.com/design/rr8F0NGA6RNY9Z0cNeh3jh/IDS-AI-Design-Library--Test-?node-id=13231-123761&m=dev (`13231:123761`) |
| Element states | https://www.figma.com/design/rr8F0NGA6RNY9Z0cNeh3jh/IDS-AI-Design-Library--Test-?node-id=13231-109521&m=dev (`13231:109521`) |
| Live evidence (2026-05-09) | File `rr8F0NGA6RNY9Z0cNeh3jh`; nodes `42266:95085`, `13231:123761`, `13231:109521`, `13231:109520`; method **Figma MCP** (`get_metadata`, `get_variable_defs`) |
