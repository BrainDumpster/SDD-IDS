...
| Verification method | **Figma REST API** (collab server-packaged evidence) — 2026-07-27. Client used packaged `tools.get_metadata`, `tools.get_design_context`, `tools.get_variable_defs`, `tools.get_screenshot`, `slotGeometry` only (no client Figma MCP). |
...
| Variant children in metadata tree | 12 COMPONENTS under set | Packaged `structure` / `get_metadata` | **partial** — Off Default + Off Hover Left/Top named in layout bullets but **absent** from returned children list (see Source Mapping) |
...
| Track Off hover fill | `var(--color-background-gray-neutral-darker)` | `#252525` |
...
| Screenshot (Main bucket) | Packaged image for `8505:14389` |
| Verification method | Figma REST API (collab packaged) — 2026-07-27 |
| Tools used (packaged) | `get_screenshot`, `get_metadata`, `get_design_context`, `get_variable_defs` (empty), `slotGeometry` |
...
| Limitation | `get_variable_defs` empty; structure lists **12** variant components while layout bullets also name Off Default/Hover Left+Top — state colors for those rows taken from packaged color list + bullet names |
