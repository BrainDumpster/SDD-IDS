...
| Verification method | **Figma REST API** (collab server-packaged evidence) — session `2oKWtpmrvFVZHAbKniH_2aMZOp7t44g7`, 2026-07-27. Client used packaged `tools.get_metadata`, `tools.get_design_context`, `tools.get_variable_defs`, `tools.get_screenshot`, `slotGeometry` only (no client Figma MCP). |
...
| Variant children in metadata tree | 12 COMPONENTS returned (+ `childrenTruncated: 4`) | Packaged `structure` / `get_metadata` | **partial** — Off Default + Off Hover Left/Top named in `get_design_context` layout + anatomy bullets (115×40 / 60×56) but truncated from returned component-set children (see Source Mapping) |
...
| Verification method | Figma REST API (collab packaged) — session `2oKWtpmrvFVZHAbKniH_2aMZOp7t44g7` — 2026-07-27 |
...
| Limitation | `get_variable_defs` empty; structure returns **12** variant components with `childrenTruncated: 4` while layout/anatomy bullets also name Off Default/Hover Left+Top — Off default/hover colors from packaged color list (`#616161` / `#252525`) + bullet dimensions |
