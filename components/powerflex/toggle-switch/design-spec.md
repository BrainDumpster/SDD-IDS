...
| Verification method | **Figma REST API** (collab server-packaged evidence) — session `UtXgC3t3PdRYPtA5-iVnRbKHR24oOkBL`, 2026-07-27. Client used packaged `tools.get_metadata`, `tools.get_design_context`, `tools.get_variable_defs`, `tools.get_screenshot`, `slotGeometry` only (no client Figma MCP). |
...
| Verification method | Figma REST API (collab packaged) — session `UtXgC3t3PdRYPtA5-iVnRbKHR24oOkBL` — 2026-07-27 |
