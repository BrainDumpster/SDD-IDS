...
export const IDS_DATAGRID_DATETIME_MODE_LABELS: Record<
  IdsDatagridDateTimeFilterMode,
  string
> = {
  all: "All",
...
export const IDS_DATAGRID_DATETIME_FILTER_MODES: IdsDatagridDateTimeFilterMode[] = [
  "all",
  "last-24-hours",
  "last-week",
  "last-month",
...
export function isIdsDatagridDateTimeFilterActive(
  ...
  if (state.mode === "specific-date") return Boolean(state.specificDate);
...
export function matchesIdsDatagridDateTimeFilter(
  cellValue: unknown,
  state: IdsDatagridDateTimeFilterState,
  now: Date = new Date(),
): boolean {
