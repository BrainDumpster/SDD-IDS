/**
 * IDS Tree — React implementation generated from design-spec.
 *
 * Path: `lib/react/ids/tree`
 * Source: `components/ids/tree/design-spec.md`
 * Theme: `components/ids-theme.css`
 *
 * Anatomy (container):
 *   IdsTree (`role="tree"`)
 *     IdsTreeItem[]          — Mode B declarative markers
 *       IdsTreeItemLabel     — label slot (`tree-item-label`)
 *
 * Row slots (`.Tree-Element` order):
 *   TreeRowRoot → ExpandChevron? → NodeIcon? → LabelCluster(NodeLabel + CountBadge?)
 *
 * Mode A: `items: TreeNode[]`. Mode B: nested IdsTreeItem markup.
...
/** Depth 0 = Figma level 1 (16px start padding). Level-6 leaf uses 136px. */
...
export function IdsTree({
  ...
  useEffect(() => {
    ...
    if (!focusedId || !rows.some((r) => r.node.id === focusedId)) {
      const fallback =
        (defaultSelectedId &&
          rows.find((r) => r.node.id === defaultSelectedId)?.node.id) ||
        rows[0]?.node.id ||
        null;
...
function TreeRow({
  ...
  const rowStyle = { paddingLeft: paddingStart } as CSSProperties;
...
export type { IdsTreeItemProps, IdsTreeItemLabelProps } from "./IdsTreeItem";
