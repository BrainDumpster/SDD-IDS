...
def _default_repo_root() -> Path:
    ...
    # Source: apps/design-spec-portal/backend/app/config.py → parents[4]
    if len(here.parents) > 4:
        candidate = here.parents[4]
        if (candidate / "config" / "design_systems").is_dir():
            return candidate
    for path in (Path(os.environ.get("REPO_ROOT", "/workspace")), Path("/workspace"), Path.cwd()):
        if (path / "config" / "design_systems").is_dir():
            return path.resolve()
    return Path(os.environ.get("REPO_ROOT", "/workspace")).resolve()


def _default_portal_root() -> Path:
    here = Path(__file__).resolve()
    # Source or Docker: .../backend/app/config.py → portal root is parents[2]
...
class Settings(BaseSettings):
    ...
    portal_root: Path = _default_portal_root()
    ...
    # Comma-separated allowlist (parsed in post-init / env override)
