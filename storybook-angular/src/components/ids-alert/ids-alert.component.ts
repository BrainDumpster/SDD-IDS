...
import {
  ALERT_GLOBAL_SEVERITY_ICON,
  ALERT_INLINE_SEVERITY_ICON,
...
export class IdsAlertComponent implements AfterContentInit, IdsAlertContext {
  ...
  statusIconSlug(): string {
    if (this.isGlobal) {
      return ALERT_GLOBAL_SEVERITY_ICON[this.severity as AlertGlobalSeverity];
    }
    return ALERT_INLINE_SEVERITY_ICON[this.severity as AlertInlineSeverity];
  }
