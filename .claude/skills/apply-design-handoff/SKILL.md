...
You are applying a design change handed off from a Claude design-system project. The handoff package lives in `handoffs/<package-name>/` on the `handoffs/` branch (or was provided directly by the operator). This skill is design-system agnostic — the package's `HANDOFF.md` declares which design system / programme it belongs to.
...
6. **Branch & PR**:
   - Branch: `handoff/<package-name>` off `main`.
   - Commit message: `design-handoff(<design-system>): <short intent>`.
   - Open the PR with `gh pr create`, using the repo PR template. Fill in the design system, affected components, Storybook updates, and reference the handoff package.
...
The owner (or any team member) re-runs `/design-sync` against the corresponding Claude design-system project — the one named in `HANDOFF.md` — so consuming projects pick up the merged change.
